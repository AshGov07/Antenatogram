...
export async function addNewRefreshToken(token, role, id) {
    ...
    try {
        ...
        try {
            let query;
            if (role === "patient") query = `INSERT INTO patientrefreshtoken(patient_id, token) VALUES (UUID_TO_BIN(?, 1), ?)`;
            if (role === "doctor") query = `INSERT INTO doctorrefreshtoken(doctor_id, token) VALUES (UUID_TO_BIN(?, 1), ?)`;
            const [results] = await connection.query(query, [id, token]);
            if (results.affectedRows === 1) return true;
            return false;
        } catch (error) {
...
export async function findAndDelete(role, token, userid, del) {
    ...
    try {
        ...
        try {
            let query;
            if (role === "patient") query = "SELECT * FROM patientrefreshtoken WHERE patient_id = UUID_TO_BIN(?, 1) AND token = ?";
            if (role === "doctor") query = "SELECT * FROM doctorrefreshtoken WHERE doctor_id = UUID_TO_BIN(?, 1) AND token = ?";
            const [results] = await connection.query(query, [userid, token]);

            if (results.length === 0) return false;
            if (del === 1) {
                if (role === "patient") query = "DELETE FROM patientrefreshtoken WHERE patient_id = UUID_TO_BIN(?, 1) AND token = ?";
                if (role === "doctor") query = "DELETE FROM doctorrefreshtoken WHERE doctor_id = UUID_TO_BIN(?, 1) AND token = ?";
                const [results] = await connection.query(query, [userid, token]);
...
export async function getNewRefreshToken(role, token, id) {
    ...
    try {
        connection = await pool.getConnection();
        let query;
        if (role === "patient") query = "UPDATE patientrefreshtoken SET token = ? WHERE patient_id = UUID_TO_BIN(?, 1) AND token = ?";
        else if (role === "doctor") query = "UPDATE doctorrefreshtoken SET token = ? WHERE doctor_id = UUID_TO_BIN(?, 1) AND token = ?";
        else return new AuthenticationError("role not found");
